...
class BayesianQLearningAgent:
    ...
    def act(self, state):
        # Calculate epsilon based on the current number of steps
        epsilon = max(self.epsilon - 0.0001 * self.steps, 0.01)
        self.steps += 1
...
for _ in range(NUM_ITERATIONS):
    ...
    generatedStartState[0] = np.random.randint(0, 50)
    generatedStartState[1] = np.random.randint(0, 50)
        ...
        def step(self, action):
            ...
            if alreadySetReward == False:
              if self.state.tolist() in self.goal_states:
                ...
                reward = self.reward_matrix[self.state[0]][self.state[1]] + (random.randint(-5, 5)/100)
    ...
    agent = BayesianQLearningAgent(alpha=0.1, gamma=0.99, epsilon=0.1, num_states=num_states, num_actions=num_actions, prior_mean=0, prior_variance=1)
