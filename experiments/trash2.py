...
for _ in range(NUM_ITERATIONS):
        ...
        def step(self, action):
            ...
            if alreadySetReward == False:
              if self.state.tolist() in self.goal_states:
                ...
                reward = self.reward_matrix[self.state[0]][self.state[1]] + (random.randint(-25, 25)/100)
    ...
    agent = BayesianQLearningAgent(alpha=0.1, gamma=0.99, epsilon=0.1, num_states=num_states, num_actions=num_actions, prior_mean=0, prior_variance=1, adapt_alpha=True, adapt_epsilon=False)
